...
*******FV**********
************************
* --- PROCESS FIRST-VISIT (FV) 2020-21 ---
use "$RAW/2020-21/fv.dta", clear

* 1. Remove suffixes
rename (*_perv1) (*)

* 2. Harmonize Location & ID Variables
* FV 2020 uses 'distcode' (no underscore).
rename distcode district_code
rename b1q1 fsu // 2020 specific
...
* 4. Standardize Types & Names
rename (B1q2 mult no_qtr qtr) (b1q2 MULT No_qtr quarter)
...
****************************************************
* RENAME BLOCK: DAILY ACTIVITY VARIABLES (7 → 1)
****************************************************
...
* -------------------
* Day 3 (3pt5) — truncated name handled
* -------------------
...
rename b6q4_act2_3~ct2  status_day3_act2
...
save "$PREP/2020-21_fv_clean.dta", replace



* --- PROCESS RE-VISIT (RV) 2020-21 ---
use "$RAW/2020-21/rv.dta", clear

* 1. Remove suffixes
rename (*_perrv) (*)
* (Note: Sometimes the suffix might be different, check if any variables remain with _perrv)

* 2. Harmonize Location & ID Variables
* RV 2020 uses 'dist_code' and 'b1q4' for Region.
...
* 4. Standardize Types & Names to match previous years
* Rename variables to lowercase/standard names if they are uppercase
rename (B1q2 mult no_qtr qtr) (b1q2 MULT No_qtr quarter)
...
* Destring numeric variables (Age, Earnings, Weights)
destring b4q6 b4q10, replace ignore(" ") force
destring MULT NSS, replace ignore(" ") force
...
save "$PREP/2020-21_rv_clean.dta", replace
