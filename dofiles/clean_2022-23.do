...
* --- PROCESS FIRST-VISIT (FV) 2022-23 ---
use "$RAW/2022-23/fv.dta", clear

* 1. Remove suffixes
rename (*_perv1) (*)

* 2. Harmonize Location & ID Variables
* In FV 2022, it is named 'distcode' (NO underscore) - Watch out for this!
...
save "$PREP/2022-23_fv_clean.dta", replace


* --- PROCESS RE-VISIT (RV) 2022-23 ---
use "$RAW/2022-23/rv.dta", clear

* 1. Remove suffixes
rename (*_perrv) (*)

* 2. Harmonize Location & ID Variables
* In RV 2022, it is named 'dist_code' (with underscore)
rename dist_code district_code
rename b1q1 fsu  // FSU is named b1q1 in recent years
rename b4q1_pervv b4q1
* 3. Fix Capitalization & Standardize Names
* Convert to lowercase to match 2017 files (fi, b1q2, quarter)
capture rename B1q2 b1q2
capture rename (qtr visit) (quarter visit)
...
* 5. Standardize Types
* Use 'capture' in case they are already numeric, prevents crashing
capture destring b4q6 b4q10, replace ignore(" ") force
...
save "$PREP/2022-23_rv_clean.dta", replace